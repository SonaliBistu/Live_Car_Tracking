...
if(isset($_POST['add_car'])){
    ...
    $stmt = $conn->prepare("INSERT INTO cars (owner_id, car_id, car_name, car_model, driver_name, gps_device_id, status, created_at) VALUES (?,?,?,?,?,?, 'Offline', NOW())");
    $stmt->bind_param("isssss",$owner_id,$car_id,$car_name,$car_model,$driver_name,$gps_device_id);
    $stmt->execute();
...
// Fetch cars
$cars = $conn->query("SELECT * FROM cars WHERE owner_id=".$_SESSION['user_id']);
$totalCars = $cars->num_rows;
$carsOnline = $conn->query("SELECT * FROM cars WHERE owner_id=".$_SESSION['user_id']." AND status='Online'")->num_rows;
$carsOffline = $conn->query("SELECT * FROM cars WHERE owner_id=".$_SESSION['user_id']." AND status='Offline'")->num_rows;
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
.sidebar .logo{
    ...
    text-align:center;
    margin-bottom:30px;
    text-shadow:0 0 10px #ff00cc,0 0 10px #00ffff;
}
.sidebar a{
    display:block;
    color:#ccc;
...
.sidebar a.active, .sidebar a:hover{
    background:#2a3a50;
    color:#fff;
    box-shadow:0 0 8px #ff00cc,0 0 8px #00ffff;
}
.sidebar .logout{
    background:#e74c3c;
    color:#fff;
...
.main h1{
    text-align:center;
    color:#00ffff;
    text-shadow:0 0 10px #ff00cc,0 0 15px #00ffff;
    margin-bottom:25px;
}
...
.profile-pic{
    ...
    box-shadow:0 0 20px #ff00cc,0 0 20px #00ffff;
    margin:0 auto 10px;
}
.user-name{
    font-weight:bold;
...
.edit-profile-btn{
    ...
    font-size:14px;
    cursor:pointer;
    box-shadow:0 0 15px #ff00cc,0 0 15px #00ffff;
    transition:0.3s;
}
.edit-profile-btn:hover{
    transform:scale(1.05);
    box-shadow:0 0 25px #ff00cc,0 0 25px #00ffff;
}
...
.add-car form button{
    background:#3498db;color:#fff; padding:8px 15px; border:none; border-radius:6px; cursor:pointer;
}
input::placeholder{color:#ccc;}
.car-list input[type="text"]{
    width:100%; padding:8px; margin-bottom:10px; border-radius:6px; border:1px solid #555;
    background:rgba(0,0,0,0.25); color:#fff;
}
</style>
</head>
...
<div class="main">
    ...
    <!-- PROFILE SECTION -->
    <div class="user-info">
        <img src="<?= $_SESSION['user_photo'] ?>" class="profile-pic" alt="User">
    ...
    <!-- CAR STATS -->
    <div class="cards">
        <div class="card total">Total Cars<br><span><?= $totalCars ?></span></div>
        <div class="card online">Cars Online<br><span><?= $carsOnline ?></span></div>
        <div class="card offline">Cars Offline<br><span><?= $carsOffline ?></span></div>
    </div>
    ...
    <div class="add-car">
        <h2>Add New Car</h2>
        <form method="post">
            <input type="text" name="car_id" placeholder="Car Number" required>
